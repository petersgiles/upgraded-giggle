...
import { RelatedLinkActionTypes, LoadRelatedLinks, GetAllRelatedLinks,
  RelatedLinksActionFailure, GetRelatedLinksByCommitment, AddLinkToCommitment, RemoveLinkFromCommitment } from './related-link.actions'
...
export class RelatedLinkEffects {
  ...
  addLinkToCommitment$: Observable<Action> = this.actions$
    .pipe(
      ...
      switchMap((payload: any) => this.service.addItemToCommitment(payload)),
  ...
  removeLinkFromCommitment$: Observable<Action> = this.actions$
    .pipe(
      ...
      switchMap((payload: any) => this.service.removeItemFromCommitment(payload)),
      switchMap((result: any) => [
  ...
  getRelatedLinksByCommitment$: Observable<Action> = this.actions$
    .pipe(
      ...
      switchMap((payload: any) => this.service.getItemsByCommitment(payload.commitment)
        .pipe(
  ...
  constructor(private actions$: Actions, private service: RelatedLinkDataService) { }
}
