...
export const getCommitmentOverviewCommitmentsMapPoints = createSelector(
  ...
  (foc, ovmc, mps, parties) => {
    ...
    console.log('getCommitmentOverviewCommitmentsMapPoints', partyHash)
    const refinedMapPoints = (ovmc || [])
      .filter(
        (cmp: any) =>
          cmp.commitment && refinedCommitmentIds.includes(cmp.commitment.id)
      )
      .map((cmp: any) => {
        const currentcommitment = refinedCommitments[cmp.commitment.id]
        const currentParty = partyHash[currentcommitment.party.id]
        const iconName = (currentParty && currentParty.icon) || DEFAULT_PARTY_ICON
