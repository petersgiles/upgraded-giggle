...
import {
  CommitmentSubscriptionActionTypes,
...
export class CommitmentSubscriptionEffects {
  @Effect()
  getSubscriptionByCommitment$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((subscription: any) =>
      this.service.getUserSubscription(subscription).pipe(
        map(
          (result: DataResult<SubscriptionResult>) =>
            new LoadSubscriptions(result)
        ),
  ...
  constructor(
    ...
    private service: CommitmentSubscriptionDataService
  ) {}
}
