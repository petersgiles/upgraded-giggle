...
export class CommitmentActionDataApolloService
  implements CommitmentActionDataService {
  removeActionFromCommitment = (variables: {
    commitment: any;
    contact: any;
    ...
    callMutate<any>(
      ...
      (result: any) => ({
        data: { commitment: result.data.deleteCommitmentAction.id }
      })
    )
  ...
  addActionToCommitment = (
    ...
    callMutate<any>(
      ...
      (result: any) => ({
        data: { commitment: result.data.storeCommitmentAction.id }
      })
    )

  getActionsByCommitment = (
    commitment: any
  ): Observable<DataResult<CommitmentActionsResult>> =>
    callQuery<CommitmentActionsResult>(
      this.apollo,
      { query: GET_COMMITMENT_ACTIONS, variables: { commitment: commitment } },
