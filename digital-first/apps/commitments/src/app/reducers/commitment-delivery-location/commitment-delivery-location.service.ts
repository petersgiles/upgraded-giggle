...
export class DeliveryLocationService {
  ...
  public addElectorateToCommitment(
    commitment: string | number,
    electorate: string | number
  ): any {
    this.store.dispatch(
      new AddElectorateToCommitment({ commitment, electorate })
    )
  }
  public removeElectorateFromCommitment(
    commitment: string | number,
    electorate: string | number
  ): any {
    this.store.dispatch(
      new RemoveElectorateFromCommitment({ commitment, electorate })
    )
  }

  public addMapPointToCommitment(
    commitment: string | number,
    mapPoint: MapPoint
  ): any {
    this.store.dispatch(new AddMapPointToCommitment({ commitment, mapPoint }))
  }
  public removeMapPointFromCommitment(
    commitment: string | number,
    mapPoint: MapPoint
  ): any {
    this.store.dispatch(
      new RemoveMapPointFromCommitment({ commitment, mapPoint })
    )
  }
  ...
  get MapPoints(): Observable<MapPoint[]> {
    return  this.store.pipe(select(fromRoot.getCommitmentMapPoints))
  }

  get Electorates(): Observable<Electorate[]> {
    return  this.store.pipe(select(fromRoot.getCommitmentElectorates))
  }

  get Expanded(): Observable<boolean> {
    return this.store.pipe(
      select(fromRoot.getCommitmentDeliveryLocationsPanelExpanded)
    )
  }
