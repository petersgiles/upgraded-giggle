...
import { byJoinTableQuery, byCommitmentIdQuery, byIdsQuery } from '../../../services/sharepoint/caml'
import { mapCommitmentPortfolios } from './maps'

@Injectable({
    providedIn: 'root'
})
export class CommitmentPortfolioDataSharePointService implements CommitmentPortfolioDataService {
        ...
        return this.sharepoint.getItems({
            ...
            concatMap((result: any) => {
                ...
                return this.sharepoint.getItems({
                    ...
                    concatMap(mapPoints =>
                        of({
                            data: { commitmentPortfolios: mapCommitmentPortfolios(mapPoints) },
                            loading: false
                        }))
                )
            })
    ...
    addPortfolioToCommitment(variables: { commitment: number; portfolio: any; }): Observable<DataResult<{ commitment: number }>> {
        const spComment = {
    ...
    removePortfolioFromCommitment(variables: { commitment: number; portfolio: any; }) {

        const viewXml = byJoinTableQuery({ fieldA: { name: 'Commitment', id: variables.commitment }, fieldB: { name: 'Portfolio', id: variables.portfolio } })
        ...
        return this.sharepoint.getItems({
            ...
            concatMap(result =>
                this.sharepoint.removeItem({
                    listName: 'CommitmentPortfolio', id: result.id
                }).pipe(
                    concatMap(_ =>
                        of({
                            loading: false,
