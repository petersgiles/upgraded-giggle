...
describe('DisplayOrderPageComponent', () => {
  ...
  let commitmentsWithDisplayOrder$: BehaviorSubject<
    any[]
  > = new BehaviorSubject([])
    ...
    commitmentDisplayOrders: [
      {
        commitmentId: 5,
        commitment: {
          portfolioLookup: { title: 'Communications and the Arts' },
          title: 'Community Music Hubs kkkk'
        },
        displayOrder: 1
      },
      {
  ...
  beforeEach(async(() => {
      ...
      testBed.configureTestingModule({
        ...
        providers: [
          ...
          provideMockStore({
            ...
            selectors: [
              {
                selector: selectRefinedCommitmentsState,
                value: initialState.commitments
              },
              {
                selector: getCommitmentDisplayOrdersState,
                value: initialState.commitmentDisplayOrders
              },
              { selector: getUserCurrentUser, value: initialState.user },
              {
                selector: getUserCurrentUserPermissions,
                value: initialState.operations
              },
              {
                selector: getDisplayOrderListChangedState,
                value: initialState.displayOrderListChanged
              }
            ]
          })
        ]
    ...
    configureTests(configure).then(testBed => {
      ...
      mockStore.overrideSelector(
        fromDisplayOrder.getCommitmentDisplayOrdersState,
        initialState.commitmentDisplayOrders
      )
      mockStore.overrideSelector(
        fromDisplayOrder.getDisplayOrderListChangedState,
        initialState.displayOrderListChanged
      )
  ...
  it('should Display Order List Changed State as false', () => {
    mockStore.select(getDisplayOrderListChangedState).subscribe(result => {
  ...
  it('should provide a commitment without a display order', () => {
    filterCommitmentsSubscription = mockStore
