...
import { GetRefinerGroups, LoadRefinerGroups } from './refiner.actions'
...
describe('RefinerEffects', () => {
  ...
  it('should start app init', inject(
    [GetRefinerTagsGQL],
    (getRefinerTags: GetRefinerTagsGQL) => {
      let result = {
        data: {
          commitmentTypes: getCommitmentTypes().data.commitmentTypes,
  ...
  function getElectoratesLookups() {
    const data = {
      data: {
        electorates: [{ id: 1, title: 'Adelaide' }, { id: 2, title: 'Aston' }]
      }
    }
    return data
  }
  ...
  function getStatesLookups() {
    const data = {
      data: { states: [{ id: 1, title: 'SA' }, { id: 2, title: 'VIC' }] }
    }
    return data
  }
