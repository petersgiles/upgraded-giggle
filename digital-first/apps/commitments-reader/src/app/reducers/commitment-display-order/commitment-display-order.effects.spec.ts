...
import { AppNotification } from '../../../../../../libs/df-app-core/src'
...
describe('RefinerEffects', () => {
  ...
  it('should dispatch a LoadCommitmentDisplayOrders action', done => {
    let siteCommitmentDisplayOrders = {
      data: {
  ...
  it('should get display orders', inject(
    [GetSiteCommitmentDisplayOrdersGQL],
    (getSiteDisplayOrderGraphQL: GetSiteCommitmentDisplayOrdersGQL) => {
      let siteCommitmentDisplayOrders = {
        data: {
          siteCommitmentDisplayOrders: [
            {
      ...
      let displayOrders = new LoadCommitmentDisplayOrders(
        siteCommitmentDisplayOrders
      )
      ...
      expect(
        commitmentDisplayOrderEffects.getCommitmentDisplayOrder$
      ).toBeObservable(expected)
    }
  ))

  it('should update display orders', inject(
    [ApplyCommitmentDisplayOrderGQL],
    (applyCommitmentDisplayOrder: ApplyCommitmentDisplayOrderGQL) => {
      ...
      scheduler.run(() => {
        actions$ = hot('-a', { a: action })
        const expected = cold('2499ms --(bcde)', {
          b: ordListChanged,
          c: getDispOrders,
          d: refinedCommitments,
          e: msg
        })
        spyOn(applyCommitmentDisplayOrder, 'mutate').and.returnValue(
          of({
            data: {
              applyCommitmentDisplayOrder: {
                id: '30bfc792-d20f-4e91-b05d-b0028d412726',
        ...
        expect(
          commitmentDisplayOrderEffects.applyCommitmentDisplayOrder$
        ).toBeObservable(expected)
      })
      scheduler.flush()
    }
