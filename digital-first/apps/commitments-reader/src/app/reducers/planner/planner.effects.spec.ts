...
describe('PlannerEffects', () => {
  ...
  const initialState = fromPlanner.initialState
  ...
  it('should Commitments', inject(
    [CommitmentEventDataService],
    (commitmentEventDataService: CommitmentEventDataService) => {
      mockStore.setState({
        planner: { schedulerPageIndex: 3, permission: 'write' },
        overview: getCommitments()
      })
      ...
      commitmentEventDataService.getEventsByCommitments = jest.fn(
        () => response
      )
  ...
  it('should load event types', () => {
    ...
    const expected = cold('-(bc)', {
      b: new GetEventTypes(result),
      c: new GetExternalEventTypes(result)
    })

    expect(plannerEffects.getEventsReferenceData$).toBeObservable(expected)
  })

  it('should get event types', inject(
    [CommitmentEventDataService],
    (commitmentEventDataService: CommitmentEventDataService) => {
  ...
  it('should get external events where there is external event', () => {
    mockStore.setState({
      planner: { selectedExternalEventTypes: getExternalEvents() }
    })
    const result = {
      data: [
        {
    ...
    const action = new GetExternalEvents({
      permission: 'write',
      selectedExternalEventTypes: getExternalEvents()
    })
    actions$ = hot('-a', { a: action })
  ...
  it('should store event', inject(
    ...
    (commitmentEventDataService: CommitmentEventDataService) => {
      const data = {
        ...
        durationUnit: 'd',
        endDate:
          'Mon Feb 11 2019 00:00:00 GMT+1100 (Australian Eastern Daylight Time)',
  ...
  it('should return real error when permission is read', () => {
    ...
    actions$ = hot('-a', { a: action })
    const expected = cold('-(bc)', {
      b: new ErrorInPlanner('You do not have permission to edit event'),
      c: new GetCommitmentEvents(null)
    })

    expect(plannerEffects.storeCommitmentEvent$).toBeObservable(expected)
  })

  it('should remove event', inject(
    [CommitmentEventDataService],
    (commitmentEventDataService: CommitmentEventDataService) => {
      ...
      const data = {
        ...
        durationUnit: 'd',
        endDate:
          'Tue Mar 26 2019 00:00:00 GMT+1100 (Australian Eastern Daylight Time)',
        ...
        resourceId: 396,
        startDate:
          'Sun Feb 24 2019 00:00:00 GMT+1100 (Australian Eastern Daylight Time)',
        permission: 'write'
      }
