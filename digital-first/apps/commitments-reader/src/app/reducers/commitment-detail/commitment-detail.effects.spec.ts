...
operationDefaults: {displayorder: 'hide',planner: 'hide',pmchandlingadvice: 'write',pmohandlingadvice: 'write'},
 operations: {ROLE_MEMBERS: [{pmchandlingadvice: 'read', pmohandlingadvice: 'read'}],
  ...
  it('should load commitment', inject([GetCommitmentDetailGQL],(getCommitmentGQL: GetCommitmentDetailGQL)  => {
    let item = {data: {commitment:  getGQLCommitment().commitmentDetail.commitment}}
    let result = getCommitment().commitmentDetail.commitment
    const action = new GetDetailedCommitment({id: 20})
    actions$ = hot('-a', { a: action} );
    const expected = cold('-(bc)', {b: new LoadDetailedCommitment(result), c: new GetHandlingAdvices()})
    spyOn(getCommitmentGQL, 'fetch').and.returnValue(of(item))
    
    expect(commitmentDetailsEffects.loadCommitmentDetails$ ).toBeObservable(expected)
  }))

  it('should load handling advices', inject([GetHandlingAdvicesGQL],(getHandlingAdvicesGQL: GetHandlingAdvicesGQL)  => {
    ...
    spyOn(getHandlingAdvicesGQL, 'fetch').and.returnValue(of(getHandlingAdvices()))
    
    expect(commitmentDetailsEffects.getHandlingAdvices$ ).toBeObservable(expected)
  }))

  it('should update PMO handling Advice', inject([UpdatePmoHandlingAdviceCommitmentGQL],(updatePMOHandlingAdvicesGQL: UpdatePmoHandlingAdviceCommitmentGQL)  => {
    mockStore.setState({app:{config: {webId: 'C9013762-2FED-49D7-8EA2-1DD650C7010E', siteId: '52233101-86F9-46D7-BBC0-22139AF854EE', 
    header: {bookType: BookType.Red}}},
    commitmentDetail: {commitment:getCommitment().commitmentDetail.commitment, handlingAdvices:getHandlingAdvices().data.handlingAdvices }})
    
    let advice = getHandlingAdvices().data.handlingAdvices[0]
    const action = new UpdatePMOHandlingAdvice({handlingAdviceId: '5f29b516-4734-4a8a-a456-402501a7b096'})
    actions$ = hot('-a', { a: action} );
    const expected = cold('-(bc)', {b: new SetPMOHandlingAdviceResult({handlingAdvice: advice}), c: new AppNotification({ message: `PMO Handling Advice Saved` })})
    spyOn(updatePMOHandlingAdvicesGQL, 'mutate').and.returnValue(of({'data':{'updatePmoHandlingAdviceCommitment':{'id':'da70b0d8-1cc7-476e-8def-b42c5ee7795f','__typename':'MutationResultGraph'}}}))
    
    expect(commitmentDetailsEffects.updatePMOHandlingAdvice$ ).toBeObservable(expected)
  }))
...
function getGQLCommitment(){
  const data = {commitmentDetail:{
                commitment: {
                  ...
                  status: '', 
                  politicalParty: 'ALP',
                  pmcHandlingAdviceCommitments:[{handlingAdvice: {label: 'Minister to write back to Prime Minister',value: '7d7329af-08a0-4742-89d3-6660b1b35280'},
...
function getCommitment(){
  const data = {commitmentDetail:{
                commitment: {
                  ...
                  politicalParty: 'ALP',
                  portfolio: '',
                  pmcHandlingAdvice: {label: 'Minister to write back to Prime Minister',value: '7d7329af-08a0-4742-89d3-6660b1b35280'},
