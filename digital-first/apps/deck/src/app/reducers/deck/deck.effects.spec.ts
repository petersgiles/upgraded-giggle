...
describe('DeckEffects', () => {
  ...
  it('should Load Deck Items', inject(
    [DeckDataService],
    (service: DeckDataService) => {
      const action = new GetDeckItems({ parent: null })
      actions$ = hot('-a', { a: action })
      const response = cold('-a|', {
        a: { data: deckData.data, loading: false }
      })
      const expected = cold('--b', {
        b: new LoadDeck({ data: deckData.data, loading: false })
      })
  ...
  it('should Load Briefs', inject(
    ...
    (service: DeckDataService) => {
      ...
      const expected = cold('--b', {
        b: new LoadBriefs({ data: briefs, loading: false })
      })
  ...
  it('should add Deck Item', inject(
    [DeckDataService],
    (service: DeckDataService) => {
      const item = {
        id: '12',
  ...
  it('should remove Deck Item', inject(
    [DeckDataService],
    (service: DeckDataService) => {
  ...
  it('should update Deck Item', inject(
    [DeckDataService],
    (service: DeckDataService) => {
      const item = {
        id: '12',
