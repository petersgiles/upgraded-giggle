...
@Component({
  ...
  *ngIf="timeFormat=='dateFormat'" (click)="onChangeDateFormat.emit('calendar')" mdc-button dense><mdc-icon>date_range</mdc-icon>{{title}}</button>
  ...
  <button  attr.aria-label="{{title}}" title="{{title}}"
  *ngIf="timeFormat=='calendar'" (click)="onChangeDateFormat.emit('timeAgo')" mdc-button dense><mdc-icon>calendar_today</mdc-icon>{{title}}</button>
 `,
 styles: [`
  :host {
    padding-right:4px;
