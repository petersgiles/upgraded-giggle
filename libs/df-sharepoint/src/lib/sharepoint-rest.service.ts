...
export class SharePointRestApiService {
  ...
  copyFile(sourceUrl: string, destinationUrl: string): Promise<any> {
    ...
    return this.http.post(url, this.options).toPromise().then((res: Response) =>
      res.json()).catch(this.handleError)
  }
  ...
  read(listName: string, options?: any): Promise<any> {
    ...
    return this.http.get(url, this.options).toPromise().then((resp: Response) =>
      resp.json())
  }
  ...
  update(listName: string, id: string, jsonBody: any): Promise<any> {
    ...
    return this.http.post(url, data, localOptions).toPromise().then((resp: Response) =>
      resp.json())
  }
  ...
  del(listName: string, id: string): Promise<any> {
    ...
    const listUrl = this.apiUrl.replace('{0}', listName)
    const url = `${listUrl}(${id})`
    return this.http.post(url, localOptions).toPromise().then((resp: Response) =>
      resp.json())
  }
  ...
  jsonRead(listName: string): Promise<any> {
    const svc = this
    return this.getCurrentUser().then((res: any) => {
      ...
      return svc.http.get(url, svc.options).toPromise().then((res2: Response) => {
  ...
  jsonWrite(listName: string, jsonBody: any) {
    const svc = this
    return this.refreshDigest().then((resp: Response) =>
      svc.jsonRead(listName).then((item: any) => {
        // HTTP 200 OK
        if (item) {
          // update if found
